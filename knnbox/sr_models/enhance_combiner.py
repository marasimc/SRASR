...
from knnbox.sr_models.util import js_divergence_subset

class EnhancedAdaptiveCombiner(nn.Module):
    def __init__(self, 
                max_k,
                probability_dim,
                k_trainable = True,
    ...
    def get_knn_prob(self, vals, distances, neural_model_logit, device="cuda:0"):
        metak_outputs = self.meta_k_network(vals, distances)
        distances_bk = distances.detach().clone()
        ...
        if self.lambda_trainable:
            metak_outputs_lambda = self.meta_k_network.forward_lambda(vals, distances_bk, knn_prob, neural_model_logit)
            self.lambda_ = metak_outputs_lambda["lambda_net_output"]
    ...
    def dump(self, path, name_prefix):
        ...
        # step 2. save model
        torch.save(self.state_dict(), os.path.join(path, name_prefix+"adaptive_combiner.pt"))


    @classmethod
    def load(cls, path, checkpoint_prefix):
        r""" load the adaptive knn-mt from disk """
        config = read_config(path)
        adaptive_combiner = cls(**config)

        adaptive_combiner.load_state_dict(torch.load(os.path.join(path, checkpoint_prefix+"adaptive_combiner.pt")))
...
class MetaKNetwork(nn.Module):
        ...
        **kwargs,
        ...
        if lambda_trainable:
            self.distance_to_lambda = nn.Sequential(
                    nn.Linear(self.max_k*2+entropy_feature_num if self.label_count_as_feature else self.max_k+entropy_feature_num, lambda_net_hid_size),
                    nn.Tanh(),
            ...
            if self.label_count_as_feature:
                ...
                nn.init.xavier_normal_(self.distance_to_lambda[0].weight[:, self.max_k:-entropy_feature_num], gain=0.1)
                nn.init.xavier_normal_(self.distance_to_lambda[0].weight[:, -entropy_feature_num:])    # modified
        ...
        # modified
        self.js_weight = nn.Sequential(
            nn.Linear(entropy_feature_num, 32),
            nn.Tanh(),
            nn.Dropout(p=0.0),
            nn.Linear(32, entropy_feature_num),
            nn.Sigmoid()
        )
        nn.init.normal_(self.js_weight[0].weight, mean=0, std=0.01)
    ...
    def forward_lambda(self, vals, distances, knn_prob, neural_model_logit):
        neural_model_prob = F.softmax(neural_model_logit, dim=-1)  # [bs, seq_len, V]
        js_features = js_divergence_subset(neural_model_prob, knn_prob)
        js_features = self.js_weight(js_features)  
        
        if self.label_count_as_feature:
            label_counts = self._get_label_count_segment(vals, relative=self.relative_label_count)
            network_inputs = torch.cat((distances.detach(), label_counts.detach().float(), js_features.detach()), dim=-1)
        else:
            network_inputs = torch.cat(distances.detach(), js_features.detach(), dim=-1)
