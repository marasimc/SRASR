"""
in this file, we implement the transformer model with adapter module
Note: this file is modified from the original codebase of the transformer model
need only to add the adapter module in Encoder
"""
...
import time
from symbolicregression.adapters.adapter_modeling import Adapter, FeedForwardAdapter
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, input, mask=None, kv=None, use_cache=False):
        ...
        if TransformerModelwtAdapter.STORE_OUTPUTS and not self.training:
...
class TransformerModelwtAdapter(nn.Module):
    ...
    def __init__(
        ...
        if self.adapter_fn:     # only apply adapter in encoder
            assert is_encoder
        ...
        if self.adapter_fn:
            self.layer_adapters1 = nn.ModuleList()
            self.layer_adapters2 = nn.ModuleList()
        ...
        for layer_id in range(self.n_layers):
            ...
            if self.adapter_fn:
                self.layer_adapters1.append(
                    FeedForwardAdapter(in_size=self.dim)
                )
                self.layer_adapters2.append(
                    FeedForwardAdapter(in_size=self.dim)
                )
    ...
    def fwd(
        ...
        for i in range(self.n_layers):
            ...
            # adapter 1
            if self.adapter_fn:
                attn = self.layer_adapters1[i](attn)
            tensor = tensor + attn
            ...
            # FFN (+ adapter-2)
            if self.adapter_fn:
                tensor = tensor + self.layer_adapters2[i](self.ffns[i](tensor))
            else:
        ...
        ## <<<<<<<<<<<<<<<<<< code for encoder_y <<<<<<<<<<<<<<<
        if self.is_encoder:
            tensor_clip = tensor.clone()            # used for computing the latent representation of data-encode
            tensor_decode = tensor.clone()          # the output of the encoder and the input of the decoder
            glob_attn = self.glob_attn_module(tensor_clip)  # output should be B x S x 1
            z_rep = torch.bmm(glob_attn.transpose(-1, 1), tensor_clip).squeeze()

            # to regain the batch dimension
            if len(tensor_clip) == 1:
                z_rep = z_rep.unsqueeze(0)

            z_rep = self.bottleneck_module(z_rep)
            return tensor_decode.transpose(0, 1), z_rep
        ## >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    ...
    def generate(
        self, src_enc, src_len, knn_model=None, max_len=200, top_p=1.0, sample_temperature=None
    ):
        """
        ...
        while cur_len < max_len:
            ...
            if knn_model:
                decoded_scores = scores
                net_output = knn_model(tensor.unsqueeze(1), decoded_scores.unsqueeze(1))
                scores = knn_model.get_normalized_probs(net_output, log_probs=True, output_layer=self.output_layer)
                scores = scores.squeeze(1)  # (bs, n_words)
            # <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< end
    ...
    def extract_top_k(
        self, src_enc, src_len, knn_model, state, top_k, max_len=200,
    ):
        """
    ...
    def generate_beam(
        self, src_enc, src_len, knn_model, beam_size, length_penalty, early_stopping, max_len=200,
    ):
        """
        Decode a sentence given initial start.
        `x`:
        ...
        while cur_len < max_len:
            ...
            if knn_model:
                decoded_scores = scores
                net_output = knn_model(tensor, decoded_scores)
                scores = knn_model.get_normalized_probs(net_output, log_probs=True, output_layer=self.output_layer)
            # <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< end
    ...
    def generate_beam_from_state(
        ...
        while cur_len < max_len:
            ...
            if use_prefix_cache:
                state_beam = tuple(generated[:cur_len][:,0].tolist())
                if state_beam not in top_k_hash:
                    top_k_hash[state_beam] = next_words[0].tolist()
